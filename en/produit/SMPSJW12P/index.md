# Mini excavator SJW-12 P Sonca — Mini excavator from 7 050 € excl. VAT

> The SONCA SJW-12 P is the refined version of the compact 1.2-tonne range: offset boom, careful finish and hydraulic joysticks built into the seat armrests for

Sonca · Mini excavator

# Mini excavator SJW-12 P

The 1.2 t with offset boom and premium finish.

In stock
7 050 € excl. VAT
i.e. 8 460 € incl. VAT
Yellow

1,015 kg
Operating weight

1.00 m
Overall width

10 hp
Engine power

1.65 m
Digging depth

[Request a quote](/en/contact/?topic=devis&msg=Hello%2C%20I%20would%20like%20a%20quote%20for%20the%20Mini%20excavator%20SJW-12%20P%20(ref.%20SMPSJW12P).%20Please%20get%20back%20to%20me.)
[05 31 60 51 61](tel:+33531605161)

✓ 2-year warranty ✓ Delivery across France ✓ Direct importer

The SONCA SJW-12 P is the refined version of the compact 1.2-tonne range: offset boom, careful finish and hydraulic joysticks built into the seat armrests for optimal ergonomics and precision. Its 1,490 mm tail swing radius and one-metre width let it navigate the tightest spaces. Designed for demanding private individuals and well-finished residential sites, it combines technical performance with aesthetics. Imported directly by CZN Machinery, 2-year warranty, delivery across France.

Grey Edition

## The SJW-12 P in grey

Same machine, same Yanmar engine and same performance as the yellow version — only the colour changes. Grey bodywork and black canopy, produced as a limited run and in stock in Toulouse.

7 490 € excl. VAT instead of €7,050 excl. VAT for the yellow version
[Request a Grey Edition quote](/en/contact/?topic=devis&msg=Hello%2C%20I%20would%20like%20a%20quote%20for%20the%20SJW-12%20P%20mini%20excavator%20in%20Grey%20Edition%20%28%E2%82%AC7%2C490%20excl.%20VAT%29.%20Please%20get%20back%20to%20me.)
Two grey SJW-12 P units being unloaded from the container Boom and quick hitch of the grey series Strapping the machines before unloading

Photos taken as the container arrived at our Toulouse warehouse. Detailed views of the grey series are available from the colour selector at the top of the page.

## Offset boom and careful finish
The SJW-12 P stands out with its offset boom and quality finish. The reinforced offset system delivers manoeuvrability and precision in all tight spaces, while the stylish engine cover protects all the components.

#### Reinforced offset boom
Durable, precise offset system for working in the tightest spaces.

#### Built-in joysticks
Hydraulic controls built into the seat armrests for optimal ergonomics.

#### Optimised KOOP engine
KOOP KD192F engine, balancing power, reliability and ease of maintenance.

## Compact and precise
With a minimum tail swing radius of 1,490 mm and a width of just 1 metre, the SJW-12 P moves with great precision around obstacles. Ideal for gardens, inner courtyards and well-finished residential landscaping.

#### 1-metre width
Easier access to tight spaces and narrow passages.

#### 1,490 mm tail swing
Ultra-precise navigation around obstacles.

#### Complete pack
Delivered with a 40 cm bucket, canopy, toolbox and filters.

## Optimised hydraulic system
The hydraulic system, reinforced to handle the offset boom, retains great precision. The 20 L Tianjin pump and the 40 cm earthworks bucket ensure smooth, perfectly controlled movements, even during offset manoeuvres.

#### 20 L Tianjin pump
Generous flow rate for smooth movements and good productivity.

#### Reinforced 40 cm bucket
Included, sized for everyday earthworks.

#### Optional grab thumb
Optional device on the dipper arm to grab all types of material.

## Technical specifications

### General specifications

**Operating weight** : 1,015 kg

**Breakout force** : 1,200 kg

**Overall length** : 2,200 mm

**Overall width** : 1,000 mm

**Overall height** : 2,240 mm

**Track shoe width** : 170 mm

**Bucket width** : 400 mm

**Track gauge height** : 320 mm

### Working dimensions

**Max. digging depth** : 1,650 mm

**Max. digging height** : 2,580 mm

**Max. dump height** : 1,750 mm

**Ground-level digging reach** : 2,500 mm

**Max. slew radius** : 1,100 mm

**Min. tail swing radius (front)** : 1,490 mm

### Drivetrain & engine

**Engine** : KOOP KD192F

**Power** : 10.3 / 11.7 hp

**Rated speed** : 3,600 rpm

**Displacement** : 499 cc

**Fuel** : Diesel Euro 5

**Cooling** : Air

**Engine type** : Single-cylinder

**Consumption** : 275–280 g/kWh

**Hydraulic pump** : Tianjin

**Pump flow rate** : 20 L

**Fuel tank** : 5.5 L

## Interested in the Mini excavator SJW-12 P?

Request your personalised quote — fast reply, advice with no obligation.

[Request a quote](/en/contact/?topic=devis&msg=Hello%2C%20I%20would%20like%20a%20quote%20for%20the%20Mini%20excavator%20SJW-12%20P%20(ref.%20SMPSJW12P).%20Please%20get%20back%20to%20me.)
